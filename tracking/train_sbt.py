...
def main():
    args = parse_args()
    if args.mode == "single":
        train_cmd = "python lib/train/run_training_sbt.py --script %s --config %s --save_dir %s --use_lmdb %d " \
        ...
        train_cmd = "python -m torch.distributed.launch --nproc_per_node %d lib/train/run_training_sbt.py " \
