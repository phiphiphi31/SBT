...
class STARKVITActor(BaseActor):
    ...
    def forward_pass(self, data, run_box_head, run_cls_head):
        ...
        out_z, out_x = self.net(img=[ NestedTensor(template_img, template_att), NestedTensor(search_img, search_att)], mode='backbone')

        feat_dict_list.append(out_z)
        feat_dict_list.append(out_x)
